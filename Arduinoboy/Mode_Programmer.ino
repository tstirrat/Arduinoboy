...
void setProgrammerConnected()
{
  ...
  Serial.println("++ Bump keep alive");
...
{
  Serial.println("<< Replying with settings");
...
void saveSettings()
{
  Serial.println("Saving sysex bytes into memory");
  byte sysexOffset = 2;
...
void programmerCheckTimeout()
{
  if((sysexProgrammingMode || sysexProgrammingConnected) && millis() > (sysexProgrammerLastResponse + PROGRAMMER_MAX_WAIT_TIME)) {
    ...
    Serial.println("Programmer: timed out (wait)");
  }
}
...
bool versionsMatch() {
  ...
  Serial.print("Version mismatch: received ");
  Serial.print(sysexData[2]);
  Serial.print(".");
  Serial.print(sysexData[3]);
  Serial.print(" expected: ");
  Serial.print(defaultMemoryMap[MEM_VERSION_FIRST]);
  Serial.print(".");
  Serial.print(defaultMemoryMap[MEM_VERSION_SECOND]);
  Serial.println("");
  return false;
}