...
void sendMidiMessageToGameboy(byte type, byte channel, byte data1, byte data2) {
  ...
  if (shouldDropRedirectedChannelMessage(type, channel)) return;
...
/** 
 * Returns true if the input midi channel has been redirected to a different channel 
 * via a mapping. This helps determine if we should stop sending this channel message 
 * to the GB.
 */
bool isChannelMappedElsewhere(uint8_t inputChannel) {
  if (inputChannel > MGB_POLY) return false;
  ...
  if (inputChannel == MGB_PU1 && customPU1 != MGB_PU1)
    return true;
  if (inputChannel == MGB_PU2 && customPU2 != MGB_PU2)
    return true;
  if (inputChannel == MGB_WAV && customWAV != MGB_WAV)
    return true;
  if (inputChannel == MGB_NOI && customNOI != MGB_NOI)
    return true;
  if (inputChannel == MGB_POLY && customPOLY != MGB_POLY)
    return true;
...
/** 
 * Returns true for MIDI messages that use channel, and the channel is mapped to a 
 * non-default. This will drop sending it to the GB, but the MIDI thru will continue to
 * send the messages.
 */
bool shouldDropRedirectedChannelMessage(byte type, uint8_t channel) {
  // message types that have channels
  if (type >= midi::NoteOff && type <= midi::PitchBend) {
    return isChannelMappedElsewhere(channel);
  }
}